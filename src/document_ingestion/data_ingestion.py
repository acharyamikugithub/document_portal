...
class FaissManager:
    def __init__(self,index_dir:Path,model_loader:Optional[ModelLoader]=None):
        ...
        if self.meta_path.exists():
            try:
                self._meta=json.loads(self.meta_path.read_text(encoding="uitf-8")) or {"rows":{}}
            except Exception:
                self._meta={"rows":{}}
    ...
    def add_documents(self,docs:List[Document]):
        if self.vs is None:
            raise RuntimeError("call load_or_create() before_document_idempotent().")
    ...
    def load_or_create(self,text:Optional[List[str]]=None,metadatas: Optional[list[dict]]=None):
        ...
        self.vs=FAISS.from_texts(text=text,embedding=self.emb,metadats=metadatas or [])
        self.vs.save_local(str(self.index_dir))
        return self.vs
...
class ChatIngestor:
    def __init__(self,temp_base:str="data",faiss_base:str="faiss_index",use_session_dirs:bool=True,session_id:Optional[str]=None):
        try:
            ...
            self.log.error("Failed to intialize ChatIngestor",error=str(e))
    ...
    def _resolve_dir(self,base: Path):
        ...
        return base
        pass
    def _split(sel,docs:List[Document],chunk_size=1000,chunk_overlap=200) ->List[Document]:
    ...
    def built_retriever(self,uploaded_files=Iterable,*,chunk_size:int=1000,chunk_overlap:int=200,k:int=5,):
        try:
            ...
            self.log.error("Failed to built retriever",error=str(e))
            raise DocumentPortalException("Failed to built retriever",e) from e
...
class DocHandler:
    def __init__(self,data_dir:Optional[str]=None,session_id:Optional[str]=None):
        ...
        self.log.info("DocHandler intialized",session_id=self.session_id,session_path=self.session_path)
    ...
    def read_pdf(self,pdf_path:str) ->str:
        try:
            ...
            text="\n".join(text_chunks)
            self.log.info("PDf read sucessfully",pdf_path=pdf_path,session_id=self.session_id,pages=len(text_chunks))
            return text
        except Exception as e:
...
class DocumentComperator:
    """
    Save, read & combine PDFs for comparision with session-based versioning
    """
    def __init__(self,base_dir:str="data/document_compare",session_id:Optional[str]=None):
        ...
        self.log.info("DocumentCompaerator intialized",session_path=str(self.session_path))
    ...
    def save_uploaded_file(self,reference_file,actual_file):
        try:
            ...
            self.log.info("File Saved",reference=str(ref_path),actual=str(act_path),session=self.session_id)
        ...
        try:
            with fitz.open(pdf_path) as doc:
                ...
                for page_num,in range(doc.page_count):
                    ...
                    if text.strip():
                        parts.append(f"\n--- Page {page_num +1}---\n{text}")
                    self.log.info("PDF read sucessfully",file=str(pdf_path),pages=len(parts))
                    return "\n".join(parts)
        except Exception as e:
    ...
    def clean_old_session(self,keep_latest:int=3):
        try:
            ...
            raise DocumentPortalException("Error cleaning old sesions",e) from e
